...
class QuestionSet:
    ...
    @staticmethod
    def has_kind_movie(word_objects):
        """
        某类型有哪些电影
        :param word_objects:
        :return:
        ...
        for w in word_objects:
            # if w.pos == pos_movie:
            #     e = u"?g :genreName '" + word_objects[1].token + "'." \
            #                                 u"?m :hasGenre ?g." \
            #                                 u"?m :movieTitle ?x".format()
            if w.token in kind:
                if w.token == '电视':
                    e = u"?g :genreName '电视电影'." \
                        u"?m :hasGenre ?g." \
                        u"?m :movieTitle ?x".format()
                else:
                    e = u"?g :genreName '{genre}'." \
                        u"?m :hasGenre ?g." \
                        u"?m :movieTitle ?x".format(genre=w.token)
...
rules = [
    ...
    Rule(condition_num=3,
         condition=(person_entity + Star(Any(), greedy=False) + every + actor + Star(Any(), greedy=False)) | (
                 every + actor + Star(Any(), greedy=False) + person_entity + Star(Any(), greedy=False)),
         action=QuestionSet.has_actor),
    # 所有电影
    Rule(condition_num=3,
         condition=(movie_entity + Star(Any(), greedy=False) + every + movie + Star(Any(), greedy=False)) | (
                 every + movie + Star(Any(), greedy=False) + movie_entity + Star(Any(), greedy=False)),
         action=QuestionSet.has_movie),
    # Rule(condition_num=2, condition=(W(pos='所有') + Star(Any(), greedy=False) + actor + Star(Any(), greedy=False)) | (
    ...
    # 电影类型
    # Rule(condition_num=3,
    #      condition=movie_entity + genre + movie + Star(Any(), greedy=False),
    #      action=QuestionSet.has_kind_movie),
    Rule(condition_num=2,
         condition=genre + movie + Star(Any(), greedy=False),
         action=QuestionSet.has_kind_movie),
